...
class DeadLetterActorRef(app: AkkaApplication) extends ActorRef with ScalaActorRef {
...
abstract class AskActorRef(promise: Promise[Any], app: AkkaApplication) extends ActorRef with ScalaActorRef {
  ...
  protected[akka] def postMessageToMailbox(message: Any, sender: ActorRef): Unit = message match {
  ...
  def ?(message: Any)(implicit timeout: Timeout): Future[Any] =
    new KeptPromise[Any](Left(new UnsupportedOperationException("Ask/? is not supported for %s".format(getClass.getName))))(app.dispatcher)
